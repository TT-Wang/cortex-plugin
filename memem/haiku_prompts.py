...
_HAIKU_MINE_SYSTEM = (
    ...
    "≤200 chars. One rule per memory. Set to \"episodic\" for a significant dated "
    "event (see EPISODIC EVENTS below). Otherwise omit this field.\n\n"
    "EPISODIC EVENTS (kind=\"episodic\"):\n"
    "Separately from durable knowledge, emit one memory for each SIGNIFICANT dated "
    "event in the session — a decision made, a tool/library/approach switched, a bug "
    "diagnosed and fixed, a release shipped, a milestone reached. Capture WHEN (the "
    "date if the conversation states one), WHAT happened, and WHY. Title it by the "
    "event itself (e.g. 'Switched from Claude CLI to DeepSeek API for streaming + "
    "lower cost'), set kind=\"episodic\", importance 3. Be selective: only events a "
    "future session would ask 'when did X happen / what happened when' about — NOT "
    "routine progress, NOT every turn. At most 3 episodic events per session.\n\n"
    ...
    "- Routine task progress or TODO state (but a SIGNIFICANT dated event is an "
    "episodic memory — see above)\n"
    "- Completed-work logs of unremarkable steps\n"
...
_HAIKU_RECONCILE_SYSTEM = (
    ...
    "- Candidates tagged type:episodic (dated events) are distinct point-in-time "
    "records — ALWAYS ADD; never PROFILE, UPDATE, SUPERSEDE, or NOOP them"
)
