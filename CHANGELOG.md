...
- **8 MCP tools deleted (`memem/server.py`, `_build_mcp()`)** — `memory_recall`, `memory_graph`, `memory_graph_audit`, `memory_graph_rebuild`, `memory_list`, `memory_import`, `context_assemble`, and `memory_remind` are removed from the MCP surface. Equivalent functionality is available via CLI (`--assemble-context`, `graph rebuild/audit/neighbors`, `--compact-index`) or Python library. The test file `tests/test_memory_remind.py` is also deleted.

### Changed

- **Docstring diet (`memem/server.py`, all 6 remaining tools)** — MCP tool descriptions rewritten to remove repetitive prose. Total schema size reduced from 12 827 chars to 5 474 chars (57% reduction). Descriptions are now imperative and action-oriented; parameter docs kept compact. `tests/test_tool_descriptions.py` updated to assert the exact set of 6 registered tool names and presence of key trigger-phrase substrings in each tool's description (no numeric character-budget assertions).
- **Telemetry isolation (`memem/recall_log.py`, `recall_log()`)** — `MEMEM_TELEMETRY_SOURCE` env guard added with two modes: (1) when set to `"test"`, `log_recall()` and `log_citation()` return early and write nothing — rows are silently discarded (this is what `tests/conftest.py` relies on to prevent test runs from polluting production telemetry); (2) when set to any other non-empty value (e.g. `"benchmark"` in `tests/benchmark_18q.py`), rows ARE written to the same production log (`~/.memem/.recall_log.jsonl`) but tagged with a `source_tag` field. `analyze_recalls()` excludes `source_tag` rows by default (`include_tagged=True` to inspect them). `tests/test_recall_log.py` added with 10 isolation-specific tests; `tests/test_citation_attribution.py` updated to respect the guard.

### Added

- **FTS5 transcript index (`memem/transcripts.py`, `index_session()`, `rebuild_transcript_index()`, `_fts5_escape_query()`)** — `transcript_search` maintains a persistent FTS5 index at `~/.memem/transcript_fts.db` (virtual table: `transcripts`). `index_session()` indexes **one row per Q/A turn-pair** (with `user_text` / `assistant_text` stored as UNINDEXED columns and their concatenation as the indexed `content`), `DELETE`+`INSERT` per `session_id` for idempotency, called incrementally from `mine_delta.run()`. `_transcript_search_fts()` returns up to `limit` distinct turn-pairs ranked by bm25, routed through the same `_format_results` as the grep path so both markup **and content granularity** match (no whole-session blob mashup). `rebuild_transcript_index()` rebuilds from all known session files; old single-row-per-session indexes auto-migrate (DROP + recreate). FTS5 is the primary path; the grep fallback (reached when the index is empty/missing — e.g. right after upgrade, before the first mine) is **bounded** by `MAX_FALLBACK_FILE_BYTES` (10 MiB), `MAX_FALLBACK_FILES` (500), and `FALLBACK_TIME_BUDGET_S` (5.0s), logging a warning when a cap is hit so truncation is never silent. Operator-injection is escaped via `_fts5_escape_query()`. `tests/test_transcripts.py` covers roundtrip, bounded-fallback, operator-injection, output-shape + per-turn granularity parity, blank-line-in-turn handling, and idempotency.
- **`--rebuild-transcript-index` CLI flag (`memem/cli.py`, dispatches to `transcripts.rebuild_transcript_index()`)** — `python3 -m memem.server --rebuild-transcript-index` rebuilds the FTS5 transcript index from scratch.
- **Path-scoped memories (`memem/retrieve.py`, `retrieve()`)** — new `paths_context: list[str] | None` parameter; after RRF+rerank+MMR pipeline, a 1.05× `w_path` bonus is applied to any memory whose `paths:` frontmatter globs match a path in `paths_context`. Small enough not to override semantic ranking but reliably surfaces path-specific memories. `tests/test_retrieve_signals.py` covers 4 `TestPathBonus` cases.
- **`paths:` frontmatter (`memem/obsidian_store.py`, `_write_obsidian_memory()`)** — memories can carry a `paths:` frontmatter list of file-glob patterns. Written when `paths` is non-empty in the save request.
- **`paths` parameter on `memory_save` (`memem/operations.py`, `memory_save()`)** — new optional `paths: list[str]` param, stored to frontmatter via `_write_obsidian_memory`. Also exposed on the MCP `memory_save` tool (`memem/server.py`) so agents can set `paths:` directly. `tests/test_operations.py` adds 4 paths-specific tests.
- **`paths_context` wired to `active_memory_slice` (`memem/server.py`, `active_memory_slice()`)** — passes `paths_context` through to `retrieve()` so the MCP tool benefits from path-scoped ranking.
- **`extract_dominant_paths()` in mining (`memem/mine_delta.py`)** — tallies **per-occurrence** file paths from the session's raw Read/Edit/Write/NotebookEdit/Bash tool calls (not the deduplicated digest) and takes the top 3; annotates mined candidates with `paths:` when ≥2 paths each appear ≥3 times; the ADD reconcile op carries `paths:` through to the saved memory. `tests/test_path_extraction.py` covers the raw per-occurrence counting, threshold behavior, and Bash-path extraction.
- **Canary check in doctor (`memem/cli.py`, `check_canaries_in_doctor()`)** — `--doctor` now runs `retrieve()` for each of the `ALWAYS_REACHABLE` canary memories and prints `PASS`/`WARN` + a "Canary check: N/5 pass" summary. Non-blocking (advisory only; never causes `--doctor` to return exit 1).
- **Dual-engine replay (`memem/eval_replay.py`, `run_dual_engine_replay()`, `format_dual_engine_report()`)** — `--dual-engine` CLI flag runs the 18-query benchmark via two labelled paths (`hook_auto` and `tool_memory_search`) side-by-side and prints a jaccard@k / top-1 stability / latency comparison report. Both paths call `retrieve()` with byte-identical arguments; the only difference is the telemetry label override, which is cosmetic — genuine engine divergence between the two paths is future work.
- **Lessons exclusion telemetry (`memem/lessons.py`, `excluded_memory_ids_for_query()`)** — calls `recall_log.log_recall(call_type='lessons_exclusion')` to track which memories are excluded for a given query. A module-level comment at the top of `lessons.py` gates the decision of whether to keep or delete the module entirely (threshold: ≥200 citation rows + ≥14 days of production data showing no repeated-citation-but-wrong-memory pattern).
- **Feedback gate guard (`memem/feedback.py`, `update_relevance_scores()`)** — a deferred-decision comment inside `update_relevance_scores()` gates read-side wiring of the EMA relevance scores (threshold: ≥200 citation rows + ≥14 days of data); write-side EMA only is active for now.
- **Canary doctor + lessons-exclusion tests** — `tests/test_canaries.py` (canary doctor pass/warn coverage) and `tests/test_lessons.py` (lessons exclusion + telemetry coverage) added.

### Benchmark

- 18-query retrieval benchmark: **79.3% (119/150)**, mean warm latency 163 ms, cross-scope 24/8 — all acceptance gates pass (precision ≥ 0.70). This is 0.7 pt below the 80.0% recorded at v2.8.0 release, but the difference is **live-vault drift**, not a regression: the benchmark runs against the live vault (which has grown since v2.8.0 was measured), and v2.9.0 makes no ranking change that affects these queries — the only retrieval change since v2.8.0 is m5's path bonus, which is a strict no-op when `paths_context` is empty (the benchmark passes no `paths_context`). Verified: HEAD and the v2.8.0 commit produce identical rankings on the 18-query set against the same vault.
